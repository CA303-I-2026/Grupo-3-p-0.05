...
Para esta verificación, se seleccionarion ciertas variables y las primeras 10 observaciones para ilustrar que los datos se encuentran en formato *tidy*, ya que cada fila representa una observación individual correspondiente a una anualidad de la póliza de un asegurado identificado por un `ID`. Asimismo, cada columna corresponde a una variable específica, como características del asegurado, del vehículo o del historial de siniestros. Esta estructura garantiza que cada variable tenga su propia columna, cada observación su propia fila y que cada tipo de unidad de análisis se mantenga consistente, lo que facilita su manipulación, visualización y análisis estadístico posterior.
...
*Titular:* *Vehículos de turismo lideran el costo anual de siniestros, furgonetas la frecuencia*

*Nombre del hallazgo/resultado:* existen diferencias notables entre la frecuencia y severidad según el tipo de vehículo

*Resumen en una oración:* los vehículos de turismo poseen un mayor costo anual de siniestros, en compararción a los otros tipos de vehículos, mientras tanto, las furgonetas presentan una mayor frecuencia de siniestros.
...
*Evidencia:* el gráfico utilizado para este análisis corresponde a la @fig-violin-combinado de la segunda bitácora.

***Nivel analítico***

*Conexión con la pregunta de investigación:* a través de este hallazgo se puede evidenciar cierto patrón tanto en la frecuencia como en la severidad de los siniestros, uno de los componentes de la pregunta de investigación es qué factores influyen en la frecuencia y la severidad de los siniestros; con este hallazgo se puede evidenciar que el tipo de vehículo influye en la frecuencia y la severidad, donde los vehículos particulares (turismo) tienen mayor costo de siniestros por año, mientras que el tipo de vehículo de furgoneta posee mayor cantidad de siniestros por año.

*Contraste con la literatura:* en contraste con las fichas de literatura, coincide con lo que proponen Fress (2021) y Omari et al. (2018), ya que ambos trabajos plantean que se debe trabajar con la frecuencia y la severidad por separado para así asignar distribuciones diferentes según el comportamiento de cada variable.

*Lo que NO explica este resultado:* este hallazgo no responde a qué factores hacen que las furgonetas tengan mayor número de siniestros ni por qué los vehículos de turismo poseen mayor costo anual de siniestros, en comparación a los otros tipos de vehículos.

*Implicación para el siguiente paso:* el hallazgo permite incluir justificadamente al tipo de vehículo como una de las variables para modelos en la tercera bitácora; además de utilizar distribuciones cero infladas para la frecuencia dada la concentración de ceros en la variable `N_claims_year` y para el costo de los siniestros por año, se seguirá el enfoque de Omari et al. (2018), al evaluar empíricamente la distribución que ofrezca mejor ajuste.
:::
...
*Contraste con la literatura:* este resultado es consistente con lo planteado por @hiraldo2005siniestros, quienes documentan una alta concentración de ceros en datos de frecuencia de seguros y argumentan que parte de estos no responde a ausencia de riesgo, sino a la no declaración de siniestros pequeños. Asimismo, coincide con @ayuso2015modelizacion, quienes evidencian que las distribuciones clásicas como la Poisson no logran capturar adecuadamente este tipo de datos, especialmente en presencia de exceso de ceros.

*Lo que NO explica este resultado:* el hallazgo no permite diferenciar si los ceros observados corresponden a asegurados que no experimentaron siniestros o a aquellos que decidieron no reportarlos. Tampoco permite identificar los factores que explican esta acumulación de ceros, por lo que no es suficiente para definir por sí solo el modelo adecuado.

*Implicación para el siguiente paso:* en los próximos avances deberá compararse la proporción de ceros observada en `N_claims_year` y `Cost_claims_year` contra la proporción que distribuciones estándar como la Poisson o la binomial negativa, esperarían generar con los mismos parámetros. Si los datos tienen más ceros de los que esas distribuciones predicen, entonces un modelo estándar los va a ajustar mal y será necesario usar modelos inflados de ceros, como el ZINB para la frecuencia o la Poisson-inversa gaussiana cero inflada para la severidad, tal como proponen @hiraldo2005siniestros y @ayuso2015modelizacion respectivamente. Si el exceso no se confirma, un GLM convencional podría ser suficiente.
:::
...
*Método o análisis que lo produjo:* los gráficos de distribución para las variables físicas del vehículo, filtrando los registros con `Cost_claims_year` \> 900.
...
*Conexión con la pregunta de investigación:* este resultado se relaciona directamente con la parte de la pregunta que busca explicar cómo intervienen las características del vehículo en la severidad de los siniestros. La concentración de reclamos superiores a 900 euros en vehículos de características medias sugiere que el aumento en el costo no parece estar asociado únicamente con vehículos extremos o especiales, sino principalmente con vehículos de uso común, posiblemente los de turismo, los familiares o incluso los cotidianos.

*Contraste con la literatura:* el resultado coincide parcialmente con Guillén et al. (2021) y Omari et al. (2018), quienes consideran la frecuencia y la severidad como dimensiones centrales en el análisis de seguros de automóviles. Además, Alvia Párraga y Linares Giler (2024) señalan que factores como el tipo de vehículo, la edad del conductor y el contexto geográfico pueden influir en los accidentes de tránsito. Sin embargo, este hallazgo solo muestra una concentración descriptiva de reclamos superiores a 900 euros, por lo que no permite confirmar una relación causal entre las características del vehículo y la severidad del siniestro.

*Lo que NO explica este resultado:* el resultado no permite concluir que los vehículos con estas características tengan mayor riesgo de generar reclamos costosos, porque no se compara contra la distribución total de vehículos en la base. Es posible que estos rangos aparezcan más porque son los tipos de vehículos más frecuentes. Además, no controla otras variables como zona, edad del asegurado, historial de siniestros, antigüedad con la aseguradora o tipo de riesgo.
...
::: callout-note
# Ficha 4: valores atípicos en el costo anual de los siniestros y el número de siniestros por año

***Nivel descriptivo***

*Titular: Variables del costo anual de los siniestros y el número de siniestros por año evidencian valores extremos marcados*

*Nombre del hallazgo/resultado:* Las variables del costo anual de los siniestros y el número de siniestros por año presentan valores mayormente bajos o moderados, no obstante, existen algunos valores muy elevados que podrían influir en el análisis de las variables.

*Resumen en una oración:* pese a la presencia de una alta concentración de ceros en los valores de las variables mencionadas, se identificaron valores atípicos marcados, mostrando importantes diferencias entre los clientes de la cartera.

*Método o análisis que lo produjo:* en el desarrollo de la sección 3.2.2.2 *Detección de valores atípicos (outliers)*, contenida en la segunda bitácora, se realizó un conjunto de boxplots orientados a ilustrar el comportemiento de los valores de determinadas variables de interés para el proyecto, dentro de ellas, el costo anual de los siniestros y el número de siniestros reportados por año. A partir de esto, se encontró el hallazgo planteado.

*Evidencia:* el gráfico que respalda el hallazgo corresponde a la figura 3 de la segunda bitácora: *Boxplots de variables clave para la identificación de valores atípicos*.

***Nivel analítico***

*Conexión con la pregunta de investigación:* la pregunta de investigación se compone de dos elementos: la relación entre las variables de la edad del asegurado, la antigüedad con la aseguradora, el historial de siniestros y las características del vehículo, tanto con la frecuencia como con la severidad de los siniestros; y la determinación de los factores que explican el incremento abrupto en la cantidad de reclamos al momento que el costo de los mismos supera el monto de 900 euros. Como es posible notar, las variables del número de siniestros por año (frecuencia) y del costo anual de los siniestros (severidad), constituyen una parte importante en ambos elementos de la propuesta. En motivo de lo anterior, resulta fundamental comprender el comportamiento de los valores de dichas variables, con el objetivo de crear asociaciones con los comportamientos presentados por los valores de las demás variables y de esta forma encaminar el trabajo a resolver el problema planteado.

*Contraste con la literatura:* en referencia a lo planteado por Omari et al. (2018), se reafirma la importancia de manejar con cautela la selección de las distribuciones para modelar las variables de la frecuencia y la severidad de los siniestros. En concordancia con lo planteado por los autores, se resalta que las distribuciones seleccionadas deben responder a las características observadas en los datos, empleando modelos separados, puesto que una elección inadecuada puede afectar directamente la veracidad de las conclusiones construidas.

*Lo que NO explica este resultado:* el presente hallazgo solo muestra el comportamiento de los valores de las variables mencionadas, si bien representa un buen primer acercamiento a la resolución del problema, no es suficiente para identificar la posible existencia de una relación entre este par de variables con la edad del asegurado, la antigüedad con la aseguradora, el historial de siniestros y las características del vehículo; tampoco es posible determinar los factores que explican el cambio abrupto en la cantidad de reclamos cuando el monto de los mismos supera el monto de 900 euros. En conclusión, el hallazgo representa únicamente una visualización inicial, por lo que se requiere de un estudio mucho más exhaustivo para acercarse a la construcción de conclusiones pertinentes.

*Implicación para el siguiente paso:* a partir del hallazgo encontrado, resulta pertinente estudiar el comportamiento de los valores de las demás variables, con el objetivo de encontrar potenciales asociaciones entre ambos comportamientos y de esta forma, acceder a una guía inicial sobre la existencia de posibles relaciones entre las variables.
:::
...
Al explorar los datos, se descubrió la marcada presencia de valores atípicos en las variables del costo de los siniestros por año y el número de siniestros; en el caso de la variable de la edad, también se encontraron algunos valores atípicos, con edades próximas a los 100 años. Además, se reafirmó la existencia de una gran densidad de ceros en la variable del número de siniestros reportados por año, así como en el costo anual de estos. Finalmente, a partir de la visualización de los datos, se distinguieron eventualidades particulares que indican la necesidad de analizar a mayor profundidad el comportamiento de ciertas variables, por ejemplo, se identificó que la distribución del número de siniestros cuyo costo supera los 900 euros, se concentra principalmente en los años más recientes, entre 2014 y 2017. Sin embargo, lo anterior no es un indicador directo de un mayor riesgo, sino que podría generarse a causa de que la base de datos contiene una mayor cantidad de registros en dichos años. Así, el conglomerado de todo lo expuesto antes, impulsa al equipo a repensar la forma en que se abordará el problema planteado, según las características de los datos.
...
En la presente bitácora, se acudió en una única ocasión a la guía de la inteligencia artificial. Así, se detalla a continuación el respectivo registro:
