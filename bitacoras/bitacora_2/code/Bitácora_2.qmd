...
Se han seleccionado ciertas variables y las primeras 10 observaciones para mostrar que los datos se encuentran en el formato *tidy*, ya que cada fila representa una observación individual correspondiente a una anualidad de la póliza de un asegurado identificado por un ID. Asimismo, cada columna corresponde a una variable específica, como características del asegurado, del vehículo o del historial de siniestros. Esta estructura garantiza que cada variable tiene su propia columna, cada observación su propia fila y cada tipo de unidad de análisis se mantiene consistente, lo que facilita su manipulación, visualización y análisis estadístico posterior.
...
Para ello, se calcularon medidas como el número de observaciones, la cantidad de valores nulos, los mínimos, los máximos, la media, la mediana, la varianza, la desviación estándar y el coeficiente de variación. En este proceso se excluyó la columna identificadora (ID), ya que corresponde únicamente a un identificador de registros y no aporta información estadística relevante para el análisis.
